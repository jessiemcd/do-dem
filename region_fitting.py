...
def get_file_region(evt_file, time0, time1, regfile, plotfile=False, regRAunit='hourangle',
    ...
    if nofit:
        plt.savefig(Path(evt_file).parts[-2]+'/'+Path(evt_file).parts[-1][0:-4]+'_COM_region.png')
    else:
        plt.savefig(Path(evt_file).parts[-2]+'/'+Path(evt_file).parts[-1][0:-4]+'_fit_region.png')
...
def write_regfile(regfile, time, region, newfile='sample'):
    ...
    RA, DEC = nupointing.get_sky_position(time, [region.center.Tx.value, region.center.Ty.value]*u.arcsec)
    #print(RA,DEC)
...
def read_regfile(regfile, time0, time1, regRAunit):
    ...
    #time0=nupointing.convert_nustar_time(time0, leap=5, astropy_time=True, from_datetime=True)
    #time1=nupointing.convert_nustar_time(time1, leap=5, astropy_time=True, from_datetime=True)
    #offset = nupointing.RADEC_to_solar(time0, time1, c.ra.deg, c.dec.deg)
    midway = time0 + (time1-time0).to(u.s).value/2*u.s
    offset = nupointing.sky_to_offset(midway, ra.to(u.deg), dec.to(u.deg))
