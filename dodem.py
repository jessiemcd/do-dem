...
def dodem(time, bl, tr, minT=5.8, maxT=7.5, dT=0.05, 
          ...
          use_prior_prep=False, save_path='./', default_err=0.2, use_highTprep=False,
          #NuSTAR-related
    ...
    dem_path = pathlib.Path(save_path) / timestring
    if not dem_path.exists():
        dem_path.mkdir()

    #Name the file where we will store the DEM inputs and results. 
    if mc_in:
        picklefile = './'+timestring+'/'+timestring+'_'+str(minT)+'_'+str(maxT)+'_'+name+'_MC_DEM_result.pickle'
        if use_highTprep or use_prior_prep:
            gg = glob.glob('./'+timestring+'/'+timestring+'_'+str(minT)+'_*_'+name+'_MC_DEM_result.pickle')
            gg.sort(reverse=True)
    else:
        picklefile = './'+timestring+'/'+timestring+'_'+str(minT)+'_'+str(maxT)+'_'+name+'_DEM_result.pickle'
        if use_highTprep or use_prior_prep:
            gg = glob.glob('./'+timestring+'/'+timestring+'_'+str(minT)+'_'+str(7.2)+'_'+name+'_DEM_result.pickle')
            gg.sort(reverse=True)
        
    if just_prep:
        picklefile='./'+timestring+'/'+timestring+'_'+str(minT)+'_'+str(maxT)+'_'+name+'_iterative_DEM_result.pickle'
        if use_highTprep or use_prior_prep:
            gg = glob.glob('./'+timestring+'/'+timestring+'_'+str(minT)+'_'+str(7.2)+'_'+name+'_iterative_DEM_result.pickle')
            gg.sort(reverse=True)
    ...
    #========================================================================== 
        ...
        if aia:
            ...
            res = aia_dem_prep.load_aia(time, bl, tr, plot=plot, aia_exclude=aia_exclude, aia_path=aia_path, 
...
#                     chanax.append(linelabels[i])
        ...
        if nustar:
            ...
            if combine_fpm:
                res = nustar_dem_prep.combine_fpm(time, nuenergies, nustar_path, make_nustar=make_nustar,
                ...
                res = nustar_dem_prep.load_nustar(time, nuenergies, nustar_path, fpm, make_nustar=make_nustar,
                                                  gtifile=gtifile, datapath=datapath, regfile=regfile, 
...
#     print('-Simulated counts in each channel, based on DEM results + response')
    ...
    #DO DEM
    dem70o,edem70o,elogt70o,chisq70o,dn_reg70o,rmatrix\
...
#     fig = plt.figure(figsize=(9, 12), tight_layout = {'pad': 1})
    ...
    if mc_in==True:
        ...
        while mc_rounds > 0:
            ...
            #DO DEM
            dem70oMC,edem70oMC,elogt70oMC,chisq70oMC,dn_reg70oMC,rmatrixMC\
...
#             plt.semilogy(mkt,dem70oMC)
        ...
        vdr.plot_DEM(data, fill_color='lightcoral',
                              title='./'+timestring+'/'+timestring+'_'+str(minT)+str(maxT)+name+'_MC')
    ...
    vdr.plot_DEM(data, fill_color='moccasin',
                          title='./'+timestring+'/'+timestring+'_'+str(minT)+'_'+str(maxT)+name)
    return picklefile
