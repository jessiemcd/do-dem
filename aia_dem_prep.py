...
def load_aia(time, bl, tr, plot=True, aia_exclude=[], aia_path='./', method='Middle',
    ...
    if method=='Middle':
        ...
        if len(ffp)!=6 or clobber==True:
            ...
            aia_for_DEM(time, bl, tr, plot=plot, aia_path=aia_path, method='Middle', clobber=clobber)
            ffp=sorted(glob.glob(aia_path+timestring+'/'+'maps_prep_*.fits'))
            if len(ffp) > 6:
    ...
    try:
        ...
        agr_path = '/Users/jessieduncan/dems/aia_response_hissw_wrapper.pro'
        try:
