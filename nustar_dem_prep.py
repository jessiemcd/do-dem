...
def make_nustar_products(time, fpm, gtifile, datapath, regfile, nustar_path, edit_regfile=True,
    ...
    if len(evt_files_0) != 1 or clobber==True:
        ...
        edit_nuscreen(nustar_path, timestring, fpm, datapath)
        f = open(nustar_path+timestring+'/'+fpm+"nuscreen_output.txt", "w")
        screenprocess = subprocess.call(nustar_path+'run_nuscreen.sh', stdout=f)
    ...
    edit_nuproducts(nustar_path, timestring, fpm, newregfile, datapath, unphys_products=unphys_products)
    f = open(nustar_path+timestring+'/'+fpm+"nuproducts_output.txt", "w")
    productprocess = subprocess.call(nustar_path+'run_nuproducts.sh', stdout=f)
...
def load_nustar(time, eng_tr, nustar_path, fpm, make_nustar=False, gtifile='', datapath='', regfile='', 
    ...
    if edit_regfile==False:
        if use_fit_regfile:
            ...
            regfile_ = glob.glob('./'+timestring+'/*'+fpm+'06_cl_sunpos_fit_region.reg')
            regfile = regfile_[0]  
        else:
    ...
    if pile_up_corr:
        if len(arf_files_unphys) == 1 and len(rmf_files_unphys) == 1 and len(pha_files_unphys) == 1 and clobber==False:
            ...
            e_lo1, e_hi1, eff_area_u = ns_tresp.read_arf(arf_files_unphys[0])
            #print('NuSTAR effective area array (function of input photon energy, units cm^2), shape: ', eff_area.shape)
            e_lo2, e_hi2, rmf_mat_u = ns_tresp.read_rmf(rmf_files_unphys[0])
            #print('NuSTAR response matrix (input photon energy vs. PHA channel, units counts (unitless)), shape: ',
                                            #rmf_mat.shape)
            engs,cnts_u,lvtm,ontim=ns_tresp.read_pha(pha_files_unphys[0])
        else:
            remake_u=True
    ...
    try:
        ...
        agr_path = '/Users/jessieduncan/dems/fvtharray_hissw_wrapper.pro'
        try:
    ...
    if pile_up_corr:
        ...
        if adjacent_grades:
            ...
            plt.xlim([0,20])
            plt.legend()
            plt.savefig('./'+timestring+'/'+timestring+fpm+'_adjacent_pile_up.png')   
            ...
            plt.xlim([0,20])
            plt.legend()
            plt.savefig('./'+timestring+'/'+timestring+fpm+'pile_up.png')
...
def edit_nuproducts(nustar_path, timestring, fpm, regfile, datapath, unphys_products=0):
    """
    Requires shell script run_nuproducts.sh in input path (nustar_path). 
    Edits based on inputs + overwrites new version.
    """
    
    with open(nustar_path+'run_nuproducts.sh', "r+") as f:
        nuproducts = f.read()
...
def edit_gti(gti_file, newstart, newstop, newfile):
    ...
    nunewstart = nupointing.convert_nustar_time(newstart, from_datetime=True, astropy_time=True)
    nunewend = nupointing.convert_nustar_time(newstop, from_datetime=True, astropy_time=True)
